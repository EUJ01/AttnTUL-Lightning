...
def accuracy_5(pred, targ):
    ...
    """
    pred = torch.topk(
        torch.log_softmax(pred, dim=1), k=5, dim=1, largest=True, sorted=True
    )[1]
    ac = (
        torch.tensor([t in p for p, t in zip(pred, targ)]).float()
    ).sum().item() / targ.size()[0]
    return ac
...
def macro_plot(
    train_macro_p, val_macro_p, train_macro_r, val_macro_r, train_macro_f1, val_macro_f1
):
