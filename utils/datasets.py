...
class MolDataset(Dataset):
    ...
    def __init__(self, data):
        ...
        for key in data:
            for one_traj in data[key]:
                ...
                traj = [one_traj[1][0]] + [
                    one_traj[1][idx]
                    for idx in range(1, len(one_traj[1]))
                    if one_traj[1][idx] != one_traj[1][idx - 1]
                ]
                time = [one_traj[2][0]] + [
                    one_traj[2][idx]
                    for idx in range(1, len(one_traj[1]))
    ...
    def __getitem__(self, index):
        return (
            self.input_seq[index],
            self.time_seq[index],
            self.state_seq[index],
...
def collate_fn(batch):
    ...
    traj_contents = torch.LongTensor(
        [
            content + [-1] * (max_len - len(content))
            if len(content) < max_len
            else content
            for content in traj_contents
        ]
    )
    time_contents = torch.LongTensor(
        [
            content + [124] * (max_len - len(content))
            if len(content) < max_len
            else content
            for content in time_contents
        ]
    )
    state_contents = torch.LongTensor(
        [
            content + [9] * (max_len - len(content))
            if len(content) < max_len
            else content
            for content in state_contents
        ]
    )
