...
class MolLightningModule(pl.LightningModule):
    def __init__(self, LocalGcnModel, GlobalGcnModel, MolModel, local_feature, local_adj, global_feature, global_adj):
        super().__init__()
    ...
    def forward(self, input_seq, time_seq, state_seq, input_index):
        grid_emb = self.LocalGcnModel(self.local_feature.to(self.device), self.local_adj.to(self.device))
        traj_emb = self.GlobalGcnModel(self.global_feature.to(self.device), self.global_adj.to(self.device))
        
        return self.MolModel(grid_emb.to(self.device), traj_emb.to(self.device), input_seq.to(self.device), time_seq.to(self.device), state_seq.to(self.device), input_index.to(self.device))
    ...
    def configure_optimizers(self):
        ...
        optimizer_localgcn = torch.optim.Adam(self.LocalGcnModel.parameters(), lr=0.001, weight_decay=5e-4)
        optimizer_globalgcn = torch.optim.Adam(self.GlobalGcnModel.parameters(), lr=0.001, weight_decay=5e-4)
