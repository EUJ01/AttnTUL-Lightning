...
class EncoderAttentionBlock(nn.Module):
    ...
    def forward(self, input_seq, input_seq_emb):
        ...
        Returns:
            [type]: [description]
        """
        enc_outputs = self.pos_emb(input_seq_emb.transpose(0, 1)).transpose(0, 1)
...
class ElasticAttentionBlock(nn.Module):
    ...
    def forward(self, querry_emb, traj_emb):
        ...
        if self.attn_strategy == "cos":
            score = torch.matmul(querry_emb, traj_emb.T) / torch.matmul(
                torch.sqrt(torch.sum(querry_emb * querry_emb, dim=1)).unsqueeze(1),
                torch.sqrt(torch.sum(traj_emb * traj_emb, dim=1)).unsqueeze(0),
            )
...
class Embedding(nn.Module):
    ...
    def forward(self, input_seq_emb, time_seq, state_seq):
        ...
        embedding = self.tanh(
            self.fc(
                torch.cat(
                    [input_seq_emb, self.timeEmb(time_seq), self.stateEmb(state_seq)],
                    dim=-1,
                )
            )
        )
...
class MolNet(nn.Module):
    ...
    def __init__(
        ...
        self.GlobalAttentionNet = ElasticAttentionBlock(Attn_Strategy, Softmax_Strategy)
        self.classifier = nn.Sequential(
            nn.Linear(d_model * 2, d_model), nn.ReLU(), nn.Linear(d_model, user_nums)
        )
    ...
    def forward(self, grid_emb, traj_emb, input_seq, time_seq, state_seq, input_index):
        ...
        input_seq_onehot = torch.zeros(
            input_seq.shape[0], input_seq.shape[1], grid_emb.shape[0]
        ).cuda()
